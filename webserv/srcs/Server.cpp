...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/08/16 19:56:54 by hyson             #+#    #+#             */
/*   Updated: 2022/08/19 08:27:40 by sanjeon          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool Server::is_request_received_complete(int code) const
{
  return code >= 1;
}

bool Server::receive(int fd)
{
  ...
  if (is_request_received_complete(code))
  {
...
{
  int code = select(_max_fd + 1, &_recv_fds, &_send_fds, NULL, &_timeout);
...
void Server::accept_connections(void)
{
  ...
  {
    if (FD_ISSET(it->first, &_recv_fds))
    {
...
void Server::iterate_clients(void)
{
  ...
  for (Clients::iterator it = _clients.begin(); it != _clients.end(); it = back_up_it_on_erase)
  {
    int client_fd = it->first;
    back_up_it_on_erase++;
    if (FD_ISSET(client_fd, &_recv_fds) && !receive(client_fd))
    {
      close_client_connection(client_fd);
      continue;
    }
    check_timeout_disconnect(it->second);
    if (FD_ISSET(client_fd, &_send_fds) && !send(client_fd))
    {
      close_client_connection(client_fd);
      continue;
    }
