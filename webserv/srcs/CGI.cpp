...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/08/17 11:42:28 by sanjeon           #+#    #+#             */
/*   Updated: 2022/08/18 05:57:30 by sanjeon          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	for (Headers::const_iterator it = _req_context.get_headers().begin(); it != _req_context.get_headers().end(); it++)
	{
		if (!it->second.empty())
		{
...
bool CGI::is_envp_ready_to_exec(void)
{
	...
	int i = 0;
	for (Envs::const_iterator it = _env.begin(); it != _env.end(); it++)
	{
...
{
	close(cgi_pipe[R]);
	if (is_request_not_sendable(cgi_pipe[W]))
	{
		return 500;
	}
	close(cgi_pipe[W]);
