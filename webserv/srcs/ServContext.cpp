...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/08/15 08:17:37 by hyson             #+#    #+#             */
...
void ServContext::set_internal_directives(Tokens::iterator *it)
{
	...
	{
		throw ConfigException("missing left brace" + get_current_parsing_line(get_line_of_token(*it)));
	}
	while (!Parser::is_right_brace(*(++(*it))))
	{
		...
		{
			throw ConfigException("invalid directive " + **it + " detected" + get_current_parsing_line(get_line_of_token(*it)));
		}
	}
}
...
ServerNames &ServContext::get_server_names(void)
{
...
void ServContext::parse_autoindex(Tokens::iterator *it)
{
	...
	{
		throw ConfigException("autoindex cannot be set after location" + get_current_parsing_line(get_line_of_token(*it)));
	}
	if (*(++(*it)) == "on")
	{
	...
	{
		throw ConfigException("autoindex unknown value" + get_current_parsing_line(get_line_of_token(*it)));
	}
	if (!Parser::is_semi(*(++(*it))))
	{
		throw ConfigException("autoindex has sevaral values" + get_current_parsing_line(get_line_of_token(*it)));
	}
}
...
void ServContext::parse_client_max_body_size(Tokens::iterator *it)
{
	if (_is_location_started)
	{
		throw ConfigException("client_max_body_size cannot be set after location" + get_current_parsing_line(get_line_of_token(*it)));
	}
	if (!Parser::is_only_digit(*(++(*it))))
	{
		throw ConfigException("client_max_body_size unknown value" + get_current_parsing_line(get_line_of_token(*it)));
	}
	_client_max_body_size = static_cast<std::uint16_t>(std::strtod((*it)->c_str(),
																   NULL));
	if (!Parser::is_semi(*(++(*it))))
	{
		throw ConfigException("client_max_body_size has sevaral values" + get_current_parsing_line(get_line_of_token(*it)));
	}
}
...
void ServContext::parse_root(Tokens::iterator *it)
{
	if (_is_location_started)
	{
		throw ConfigException("root cannot be set after location" + get_current_parsing_line(get_line_of_token(*it)));
	}
	_root = *(++(*it));
	if (!Parser::is_semi(*(++(*it))))
	{
		throw ConfigException("root has sevaral values" + get_current_parsing_line(get_line_of_token(*it)));
	}
}
...
void ServContext::parse_upload(Tokens::iterator *it)
{
	if (_is_location_started)
	{
		throw ConfigException("upload cannot be set after location" + get_current_parsing_line(get_line_of_token(*it)));
	}
	_upload = *(++(*it));
	if (!Parser::is_semi(*(++(*it))))
	{
		throw ConfigException("upload has sevaral values" + get_current_parsing_line(get_line_of_token(*it)));
	}
}
...
void ServContext::parse_cgi_bin(Tokens::iterator *it)
{
	if (_is_location_started)
	{
		throw ConfigException("cgi_bin cannot be set after location" + get_current_parsing_line(get_line_of_token(*it)));
	}
	_cgi_bin = *(++(*it));
	if (!Parser::is_semi(*(++(*it))))
	{
		throw ConfigException("cgi_bin has sevaral values" + get_current_parsing_line(get_line_of_token(*it)));
	}
}
...
void ServContext::parse_auth(Tokens::iterator *it)
{
	if (_is_location_started)
	{
		throw ConfigException("auth cannot be set after location" + get_current_parsing_line(get_line_of_token(*it)));
	}
	_credentials = *(++(*it));
	if (!Parser::is_semi(*(++(*it))))
	{
		throw ConfigException("auth has sevaral values" + get_current_parsing_line(get_line_of_token(*it)));
	}
}
...
void ServContext::parse_cgi(Tokens::iterator *it)
{
	if (_is_location_started)
	{
		throw ConfigException("cgi cannot be set after location" + get_current_parsing_line(get_line_of_token(*it)));
	}
	std::vector<std::string> cgi_arguments;
	while (!Parser::is_semi(*(++(*it))))
	{
		cgi_arguments.push_back(**it);
	}
	if (cgi_arguments.size() != 2)
	{
		throw ConfigException("cgi has not 2 values for extension and executable" + get_current_parsing_line(get_line_of_token(*it)));
	}
...
void ServContext::parse_listen(Tokens::iterator *it)
{
	if (_is_location_started)
	{
		throw ConfigException("listen cannot be set after location" + get_current_parsing_line(get_line_of_token(*it)));
	}
	...
	{
		throw ConfigException("port is not a valid number" + get_current_parsing_line(get_line_of_token(*it)));
	}
	port = static_cast<std::uint16_t>(std::strtod(after_ip.c_str(), NULL));
	if (port > MAXIMUM_PORT_NUMBER)
	{
		throw ConfigException("port is not in range [1 - 65535]" + get_current_parsing_line(get_line_of_token(*it)));
	}
	Listen listen(ip, port);
	if (std::find(_listens.begin(), _listens.end(), listen) != _listens.end())
	{
		throw ConfigException("listen is duplicated" + get_current_parsing_line(get_line_of_token(*it)));
	}
	_listens.push_back(listen);
	if (!Parser::is_semi(*(++(*it))))
	{
		throw ConfigException("listen has sevaral values" + get_current_parsing_line(get_line_of_token(*it)));
	}
}
...
void ServContext::parse_index(Tokens::iterator *it)
{
	if (_is_location_started)
	{
		throw ConfigException("index cannot be set after location" + get_current_parsing_line(get_line_of_token(*it)));
	}
	while (!Parser::is_semi(*(++(*it))))
	{
...
void ServContext::parse_limit_except(Tokens::iterator *it)
{
	if (_is_location_started)
	{
		throw ConfigException("limit_except cannot be set after location" + get_current_parsing_line(get_line_of_token(*it)));
	}
	while (!Parser::is_semi(*(++(*it))))
	{
...
void ServContext::parse_error_page(Tokens::iterator *it)
{
	if (_is_location_started)
	{
		throw ConfigException("error_page cannot be set after location" + get_current_parsing_line(get_line_of_token(*it)));
	}
	int code;
	ErrorCodes codes;
	while (Parser::is_only_digit(*(++(*it))))
	{
		code = static_cast<std::uint16_t>(std::strtod((*it)->c_str(), NULL));
	...
	if (!Parser::is_semi(*(++(*it))))
	{
		throw ConfigException("error_page has sevaral values" + get_current_parsing_line(get_line_of_token(*it)));
	}
}
...
void ServContext::parse_server_name(Tokens::iterator *it)
{
	if (_is_location_started)
	{
		throw ConfigException("server_name cannot be set after location" + get_current_parsing_line(get_line_of_token(*it)));
	}
	while (!Parser::is_semi(*(++(*it))))
	{
