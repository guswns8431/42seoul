...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/08/12 14:42:17 by hyson             #+#    #+#             */
/*   Updated: 2022/08/18 06:02:11 by hyson            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	std::size_t left_result = trim_right(line);
	std::size_t right_result = trim_left(line);
...
bool Parser::is_semi(const std::string &str)
{
	return str == ";";
}
...
void Parser::parse_workers_directive(Tokens::iterator it)
{
	...
	{
		throw ParserException("workers not a valid number" + get_current_parsing_line(get_line_of_token(it)));
	}
	_worker_count = static_cast<std::size_t>(std::strtod(it->c_str(), NULL));
	if (!is_workers_in_range())
	{
		throw ParserException("workers not in a valid range [1 - 5]" + get_current_parsing_line(get_line_of_token(it)));
	}
}
...
void Parser::parse_top_directives(void)
{
	for (Tokens::iterator it = _tokens.begin(); it < _tokens.end(); it++)
	{
		...
		{
			throw ParserException("unknown directive " + *it + " detected" + get_current_parsing_line(get_line_of_token(it)));
		}
	}
}
