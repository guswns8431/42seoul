...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/08/15 11:17:42 by sanjeon           #+#    #+#             */
/*   Updated: 2022/08/18 04:35:09 by sanjeon          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool Response::case_on_GET_or_HEAD_with_return_possible(void)
{
	if (_file.is_directory()) 
	{
		...
		{
			_is_redirected = true;
			_redirected_target = ft::get_sole_slash("/" + _req_ctx.get_target() + "/" + index);
			_status_code = 200;
			return true;
		}
...
void Response::case_on_POST_or_PUT(void)
{
	...
	if (!_req_ctx.get_upload().empty())
	{
		...
		path = _req_ctx.get_uri() + "/" + _req_ctx.get_upload() + "/" + _req_ctx.get_resource();
...
bool Response::is_authenticated(void) const
{
	...
	std::string token = cred.substr(cred.find(' ') + 1);
...
int Response::GET()
{
	...
	{
		_headers["Content-Type"] = Engine::mimes.get_type(".html");
		_body = _file.get_autoindex_page(_req_ctx.get_target());
		_headers["Content-Length"] = ft::to_string(_body.length());
	}
	else
	{
...
int Response::PUT()
{
	...
	else if (_req_ctx.get_body().empty() ||
